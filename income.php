...
<section class="section-1">
 <body>
    <main class="table" style="width: 1000px;">
        <section class="table__header">
            <h1>Income </h1>
        </section>


                            <button onclick="printTable()" style="width: 80px; height: 40px;  background: linear-gradient(45deg, green, #243b55);border-radius: 7px; margin-left: 800px;">Print</button>
                            <form method="post" action="">
    <label for="month" style="color:white; margin-right: -20px;">Select Month:</label>
    <select name="month" id="month" style="width: 80px; height: 40px;  background: linear-gradient(45deg, white, #243b55);border-radius: 7px; margin-left: 20px;" >
        <?php
        $months = [
            "01" => "January",
            "02" => "February",
    ...
    <input type="submit" value="Filter" style="width: 80px; height: 40px;  background: linear-gradient(45deg, green, #243b55);border-radius: 7px; margin-left: 20px;">
</form>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['month'])) {
    ...
    // Get transactions and total income for the selected month
    $sql = "SELECT transaction_id, created_at, tracking_code, amount, fee, status FROM transactions WHERE status = 'received' AND DATE_FORMAT(created_at, '%m') = ?";
    ...
    $sql = "SELECT transaction_id, created_at, tracking_code, amount, fee, status FROM transactions WHERE status = 'received' AND DATE(created_at) = '$currentDate'";
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<p>No income data available for the current date.</p>";
    }
}
...
<style>
    ...
    #data-table th, #data-table td {
        padding: 12px 20px;
        border-bottom: 1px solid #2C6E44;
        text-align: left;
    }
...
<p>Total Income:₱<?php echo $totalFee; ?>.00</p>
		</div>
